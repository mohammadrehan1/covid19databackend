...
class CovidTestController {
	...
	public void covidTest() throws Exception {
		...
		Mockito.when( rs.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<CovidReport>>() {
		})).thenReturn(new ResponseEntity<List<CovidReport>>(list, HttpStatus.OK));
		ResponseEntity<List<CovidReport>> list3 = covid19DataServiceImpl.getCovid19Data();
